...
// --- 🌿 REAL DATA DICTIONARY (50+ Plants) ---
const realPlants = [
  // IMMUNITY
...
// Generate 300 Lines
for (let i = 0; i < 300; i++) {
  const plant = realPlants[i % realPlants.length]; // Cycle through real plants
  
  // Image Logic: Use reliable Placehold.co images with Text so they ALWAYS show
  // Colors change based on category for fun
  const color = i % 2 === 0 ? "22c55e" : "166534"; // Green shades
  const imgUrl = `https://placehold.co/600x400/${color}/ffffff?text=${plant.name.replace(" ", "+")}`;
  
  const row = [
    plant.cat,
    `${plant.name}`,
    `"${plant.desc}"`, // Quotes for commas in text
    `"${plant.use}"`,
    `"${plant.adv}"`,
    `"${plant.dis}"`,
    `"${plant.side}"`,
    imgUrl, imgUrl, imgUrl, imgUrl, imgUrl // 5 Images
  ].join(",");
